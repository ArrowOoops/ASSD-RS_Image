...
class DEC_Module(object):
            ...
            for phase in ['train', 'val']:
                if phase == 'train':
                    ...
                    for data in dset_loaders:
                        ...
                        # "epoch=%s"%(int(epoch)+1)+      "第"+str(i_loss)+"次迭代，
                        i_loss += 1
                        print("epoch=" + str(epoch + 1) + "第" + str(i_loss) + "次迭代，loss是：" + str(loss.item()))
    ...
    def test(self):
        ...
        for i in range(num_imgs):
            ...
            for j in range(1, detections.size(1)):#[1,objcts]
                dets = detections[0, j, :]
    ...
    def eval(self, device, eval_model, dsets):
        ...
        for i in range(num_imgs):
            print("System is processing {}th image".format(i+1))
            img, target = dsets.__getitem__(i)
            ori_img = cv2.imread(dsets._imgpath % dsets.img_ids[i])
            h, w, c = ori_img.shape
