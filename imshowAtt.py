...
def imshowAtt(beta, img=None):
    ...
    if img is None:
        img = cv2.imread(os.path.join(cfg.root,"VOC13/JPEGImages/000001.jpg"), 1) # the same input image
        #img = cv2.imread(os.path.join(cfg.root, "VOC2007/JPEGImages/000358.jpg"), 1)  # the same input image
    h,w,c = img.shape
    img1 = img.copy()
